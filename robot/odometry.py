...
while net.receive():  # Wait for messages and timer ticks.
    ...
    if net.id == 'Timer':                                                # If timesr has come:
        ...
        vel_west      += TIMER * ( acc_x * cosd(yaw) + acc_y * sind(yaw))  # Calculate new
        vel_north     += TIMER * (-acc_x * sind(yaw) + acc_y * cosd(yaw))  # velocity.
        sensor.pos[X] += TIMER * vel_west                                  # Update position in
        sensor.pos[Y] += TIMER * vel_north                                 # outgoing message.
